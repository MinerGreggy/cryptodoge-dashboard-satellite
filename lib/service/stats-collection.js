...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'cryptodoge-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('cryptodoge').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('cryptodoge')[service]);
